...
def main():
    ...
    hp = {'lr': 0.3, 'pk': 0.04, 'w_lr': 0.4}

    image_path = "./DAVIS/JPEGImages/480p/tennis/"
    ann_path = "./DAVIS/Annotations/480p/tennis/"
    ...
    for frame in get_frames(image_path):
        ...
        if first_frame:
            ...
            cv2.rectangle(frame, (bbox[0] - bbox[2] // 2, bbox[1] - bbox[3] // 2),
                          (bbox[0] + bbox[2] // 2, bbox[1] + bbox[3] // 2),
                          (0, 255, 0), 3)
            coords = get_contours_from_polar(outputs['mask_cen'][0], outputs['mask_cen'][1], outputs['mask'])
            for i in range(len(coords)):
                cv2.line(frame, (int(outputs['mask_cen'][0]), int(outputs['mask_cen'][1])), (int(coords[i][0]), int(coords[i][1])),(0, 255, 0), 1)
