...
class COCODataset(data.Dataset):
    ...
    def transform_cords(self, image, bbox, segmentation_in_original_image):
      ...
      for i in range(len(segmentation_in_padding_img[0])//2):
        ...
        segmentation_in_cropped_img[0][2*i] = np.clip(segmentation_in_cropped_img[0][2*i], 0, x12 - x11).astype(np.float32)
        segmentation_in_cropped_img[0][2*i+1] = np.clip(segmentation_in_cropped_img[0][2*i+1], 0, y12 - y11).astype(np.float32)
